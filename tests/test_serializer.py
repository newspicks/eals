...
def test_serialize_and_deserialize_without_compression(tmp_path):
    # 3 users x 2 items
    ...
    file = (tmp_path / "model.json.gz").as_posix()
    serialize_json(file, model)
    model_actual = deserialize_json(file)
